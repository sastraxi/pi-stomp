...
(`show_eq_panel`, `universal_encoder_select`, `universal_encoder_sw`,
`consume_tweak_rotation`) and capture the LCD frame at each step.
...
from pistomp.eq import FIL4_MONO_URI
...
def make_fil4_plugin(instance_id: str = "fil4") -> Plugin:
    ...
    for b in BANDS:
        ...
        # geometric mean of the band's freq range as a neutral default
...
def tweak(handler, idx: int, rotations: int) -> bool:
    """Drive a tweak encoder rotation through the intercept hook."""
    return handler.consume_tweak_rotation(idx, rotations)
...
def test_eq_volume_passthrough_on_chrome(v3_system: SystemFixture):
    """When Back/Bypass/Reset is focused, Tweak3 must NOT be consumed."""
    handler = v3_system.handler
    open_eq(v3_system)

    # Tweak3 on a band → consumed
    assert tweak(handler, 3, 1) is True

    # Nav past all 8 bands to Back (HP→LS→B1→B2→B3→B4→HS→LP→Back = 8 steps)
    nav(handler, 8)

    # Tweak3 now falls through
    assert tweak(handler, 3, 1) is False
    # Tweak1/2 still consumed silently on chrome
    assert tweak(handler, 1, 1) is True
    assert tweak(handler, 2, 1) is True
